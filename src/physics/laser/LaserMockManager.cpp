...
LaserMockManager::LaserMockManager(shared_ptr<Loader> ldr,
                           shared_ptr<GridManager> gridMnr,
                           shared_ptr<Pusher> pshr,
                           shared_ptr<ClosureManager> cm):loader(move(ldr)),
                           gridMgr(move(gridMnr)), pusher(move(pshr)){
                                
    logger.reset(new Logger());
    envelopeEqSolver.reset(new EnvelopeEquationSolver(loader,gridMgr,pusher,cm));
    initialize();
        ...
        for( j = 0; j < yRes; j++ ){
            for( k = 0; k < zRes; k++ ){
                ...
                if( pres > 0.0 ){
                    ...
                    desireDens = 0.1*targetIonDensityProfile[idxOnG2];// - dens4Injected[idxOnG2]->getValue()[0];
                }else{
...
void LaserMockManager::moveBeamEnvelope(int phase, int i_time){
    envelopeEqSolver->solve(phase, i_time);
}
