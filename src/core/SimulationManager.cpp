...
SimulationManager::SimulationManager(int ac, char **av) {
    ...
    if(rank == 0){
        ...
        logger->writeMsg("****                                                   ****", INFO);
        usleep(fancyStart);
        logger->writeMsg("****       O        0  A       A         M          M  ****", INFO);
        usleep(fancyStart);
        logger->writeMsg("****      O O       O  A      A A        M M      M M  ****", INFO);
        usleep(fancyStart);
        logger->writeMsg("****     O   O      O A      A   A       M M      M M  ****", INFO);
        usleep(fancyStart);
        logger->writeMsg("****    OOOOOOO     0A      AAAAAAA      M   M   M  M  ****", INFO);
        usleep(fancyStart);
        logger->writeMsg("****   O       0    O A    A       A     M     M    M  ****", INFO);
        usleep(fancyStart);
        logger->writeMsg("****  O         O   O  A  A         A    M          M  ****", INFO);
        usleep(fancyStart);
        logger->writeMsg("****   Arbitrary  Kinetic  Algorithm           M       ****", INFO);
        usleep(fancyStart);
        logger->writeMsg("***********************************************************", INFO);
        usleep(fancyStart);
...
void SimulationManager::initialize() {
    ...
    laserMng.reset(new LaserMockManager(loader, gridMng, pusher, closureMng));
    elemagMng.reset(new EleMagManager(loader, gridMng));
    collideMng.reset(new IonIonCollisionManager(loader, gridMng, pusher));
    eicollideMng.reset(new ElectronIonCollisionManager(loader, gridMng, pusher, closureMng));
    ionizeMng.reset(new IonizationManager(loader, gridMng, pusher));
    solver.reset(new Solver(loader, gridMng, pusher,
                            hydroMng, elemagMng, closureMng,
                            laserMng, collideMng, eicollideMng, ionizeMng));    
