...
const string  BRACKETS_4DOUBLE = "(dddd)";
...
const string  GET_LASER_BEAM_ENVELOPE  = "getLaserBeamEnvelopeProfile";

const string GET_POTENTIAL_THRESHOLD = "getPotentialThreshold4Interaction";
const string GET_CRITICAL_DENSITY = "getCriticalDensity";


const string IF2USE_IONIZATION = "getIfWeUseIonization";
const string IF2USE_EI_COLLISION = "getIfWeUseElectronIonCollisions";
const string GET_EI_COLLISION_FREQUENCY = "getElectronIonCollisionFrequency";
...
complex<double> Loader::callPyFloatFunctionWith4args( PyObject* instance,
                                            const string funcName,
                                            const string brackets,
    ...
    if (checkMethodExistence(funcName, brackets) == METHOD_OK){
        ...
        PyObject* obj = PyObject_CallMethod(instance,strdup(funcName.c_str()),
                                                strdup(brackets.c_str()),x,y,z,time);
        Py_complex value = PyComplex_AsCComplex(obj);
        complex<double> imaj(0.,1.);        
        return  value.real + imaj*value.imag;
...
PyObject* Loader::getPyMethod(PyObject* instance,
                               const string funcName,
                               const string brackets){
    PyObject* result = PyObject_CallMethod(instance, strdup(funcName.c_str()), strdup(brackets.c_str()));
    if (!result) {
        PyErr_Print();
        return nullptr;
    }
...
void Loader::load(){
    ...
    this->criticalDensity = callPyFloatFunction( pInstance, GET_CRITICAL_DENSITY, BRACKETS );

    this->potentialThreshold4Interaction = callPyFloatFunction( pInstance, GET_POTENTIAL_THRESHOLD, BRACKETS );
    
    this->useIonization = (int) callPyLongFunction( pInstance, IF2USE_IONIZATION, BRACKETS);

    this->useEIcollision = (int) callPyLongFunction( pInstance, IF2USE_EI_COLLISION, BRACKETS);

    this->eiCollFreq = callPyFloatFunction( pInstance, GET_EI_COLLISION_FREQUENCY, BRACKETS );
    ...
    if( rank == 0 ){
        ...
        msg = "[Loader] [COMMON] run from "+msg+" "+to_string(this->dim)+"D simulation";
        logger.writeMsg(msg.c_str(), INFO);
        ...
        msg = "[Loader] [ENVELOPE]: potential theshold = "+to_string(potentialThreshold4Interaction);
        logger.writeMsg(msg.c_str(), INFO);

        msg = "[Loader] [ENVELOPE]: critical density = "+to_string(criticalDensity);
        logger.writeMsg(msg.c_str(), INFO);

        if( useIonization == 1){ 
            msg = "[Loader] [Ionization]: ON ";
            logger.writeMsg(msg.c_str(), INFO);
        }else{
            msg = "[Loader] [Ionization]: OFF ";
            logger.writeMsg(msg.c_str(), INFO);
        }

        if( useEIcollision == 1 ){ 
            msg = "[Loader] [EIcollisions]: ON, collision frequency = "+to_string(eiCollFreq);
            logger.writeMsg(msg.c_str(), INFO);
        }else{
            msg = "[Loader] [EIcollisions]: OFF ";
            logger.writeMsg(msg.c_str(), INFO);
        }
...
complex<double> Loader::getLaserBeamEnvelopeProfile( double x, double y, double z, double time ){
    string varName = GET_LASER_BEAM_ENVELOPE;
    return callPyFloatFunctionWith4args( pInstance, varName, BRACKETS_4DOUBLE, x, y, z, time );
}
