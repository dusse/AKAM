        ...
        for( j = 0; j < yRes; j++) {
            for( k = 0; k < zRes; k++){
                ...
                for( spn = 0; spn < numOfSpecies; spn++ ){
                    ...
                    //        string msgi220011 ="[Mod] idx = "+to_string(idxOnG2) + " requiredPrtclNum = "+to_string(requiredPrtclNum);
                    //        logger->writeMsg(msgi220011.c_str(),  DEBUG);
                    ...
                    for( ptclIDX = 0; ptclIDX < requiredPrtclNum; ptclIDX++ ){
                        ...
                        if( distributionType == 0 ){
                        ...
                        else if( distributionType == 1 ){ //linear
                            double r1 = RNM;
                            double r2 = RNM;
                            double theta = acos(1.0 - 2.0*r1);  // polar
                            double phi   = 2.0 * M_PI * r2;     // azimuth

                            // Triangular speed distribution (v ~ v_max * sqrt(r))
                            double r3 = (ptclIDX + 0.5) / requiredPrtclNum;   // evenly spaced in (0,1)
                            double vmag = vel[0] * sqrt(r3);

                            // Velocity components
                            vpb[0] = vmag * sin(theta) * cos(phi) + fluidVel[0];
                            vpb[1] = vmag * sin(theta) * sin(phi) + fluidVel[1];
                            vpb[2] = vmag * cos(theta)            + fluidVel[2];

                        }else if( distributionType == 2 ){//SUPER GAUSS
                            double r1 = RNM;
                            double r2 = RNM;
                            // angles
                            double theta = acos(1.0 - 2.0*r1);   // polar
                            double phi   = 2.0 * PI * r2;        // azimuth
                            
                            // radial speed from super-Gaussian (power 4)
                            double r3 = RNM;   // uniform in [0,1]
                            double vtrial, rtrial, vmag;
                            do {
                                vtrial = vel[0] * RNM;           // trial velocity
                                rtrial = RNM;
                            } while( rtrial > pow(vtrial/vel[0],2) * exp(-pow(vtrial/vel[0],4)) );
                            vmag = vtrial;
                            ...
                            double theta = acos(1.0 - 2.0*r1);   // polar angle
                            double phi   = 2.0 * PI * r2;        // azimuth
                            
                            double r3 = RNM;
                            double vmag = vel[0] * r3;   // uniform in [0, vmax]
                        ...
                        if( loader->useIonization == 1 ){
                            pusher->setParticleCharge(particle_idx, MIN_CHARGE);
                        }else{
                            pusher->setParticleCharge(particle_idx, pusher->getParticleCharge4Type(spn));
                        }
                        particle_idx++;
                    }
                }
