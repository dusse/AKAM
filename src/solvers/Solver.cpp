...
Solver::Solver(shared_ptr<Loader> load,
               ...
               shared_ptr<IonIonCollisionManager> cm,
               shared_ptr<ElectronIonCollisionManager> eicm,
               shared_ptr<IonizationManager> ionm):loader(move(load)),
                            gridMng(move(grid)), pusher(move(pshr)),
                            hydroMng(move(hydro)), emMng(move(em)),
                            closureMng(move(closure)), laserMng(move(lm)),
                            collideMng(move(cm)),
                            eicollideMng(move(eicm)), ionizeMng(move(ionm)){
...
void Solver::performCalculation(int PHASE, int i_time){
    ...
    if( loader->useIonization == 1 ){
        ionizeMng->calculateIonizationLevel(PHASE);
    }
    hydroMng->gatherMoments(PHASE);
    if( loader->useEIcollision == 1 ){
        eicollideMng->collideElectronWithIons(PHASE);
    }
    ...
    if( loader->useEIcollision == 1 ){
        eicollideMng->applyElectronPressure(PHASE);
    }

    laserMng->moveBeamEnvelope(PHASE, i_time);
